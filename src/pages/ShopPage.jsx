...
const ShopPage = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(""); // Add this state for category filtering
  const [showCategoryMenu, setShowCategoryMenu] = useState(false); // Toggle visibility of category menu
        ...
        <div className="w-full md:w-1/4 border border-gray-200 p-6 rounded-lg shadow">
          ...
          <div className="mt-6">
            <button
              ...
              className="w-full bg-pink-500 hover:bg-pink-900 text-white font-semibold py-2 rounded mt-4 transition"
            >
              Shop By Category
            </button>
            {showCategoryMenu && (
              <div className="mt-4">
                <button
                  onClick={() => { setSelectedCategory('new'); applyFilters(); setShowCategoryMenu(false); }}
                  className="block w-full text-left py-2 px-4 text-gray-800 hover:bg-gray-200"
                >
                  New
                </button>
                <button
                  onClick={() => { setSelectedCategory('popular'); applyFilters(); setShowCategoryMenu(false); }}
                  className="block w-full text-left py-2 px-4 text-gray-800 hover:bg-gray-200"
                >
                  Popular
                </button>
                <button
                  onClick={() => { setSelectedCategory('trending'); applyFilters(); setShowCategoryMenu(false); }}
                  className="block w-full text-left py-2 px-4 text-gray-800 hover:bg-gray-200"
                >
                  Trending
                </button>
  ...
  {filteredProducts.map(({ title, price, oldPrice, imgUrl, onSale }, index) => (
    <div key={index} className="bg-white border rounded-lg overflow-hidden shadow group">
      ...
      <div className="p-4 text-center">
        ...
        <button
          onClick={() => alert(`Buying ${title}`)} // You can handle the buying logic here
          className="mt-4 bg-pink-500 hover:bg-pink-600 text-white font-semibold py-2 px-4 rounded transition"
        >
          Buy Now
        </button>
      </div>
